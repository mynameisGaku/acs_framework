...
| 決定性 (記録と再生) | `CSimulationSubsystem`、`CDeterministicRandom`の確率判定、均等・重み付き抽選、配列シャッフル、3D箱内部・三角形面・円盤内部・円柱内部・カプセル内部・球面・球内部・円錐方向抽選 |
