...
配置後は5組と共通親を先に検証し、同じ形状番号のまま床上面、内寸、壁と床の寸法、材質、レイヤーを
同期更新する。
...
`TryApplyTo`は全3組の所有、重複、共通親、祖先を先に検証し、ノード・形状番号を保って入口、方向、
寸法、見た目、レイヤーを同期更新する。一括破棄でも全3組の所有関係と重複を先に検証する。
...
途中失敗は逆順に巻き戻し、同期更新では全3組の所有、生存、共通親、重複を、一括破棄では
所有関係と重複を先に検証する。
