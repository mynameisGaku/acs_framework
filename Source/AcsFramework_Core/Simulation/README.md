...
出現位置、探索候補、破片などを直方体範囲へ散らす場合は、原点中心のローカルoffsetを
`TryPointInBox3D()`で作る。半寸法はX、Y、Z軸ごとに指定し、利用側の中心位置へ加える。
回転した範囲が必要なら、返ったoffsetへ利用側のローカル回転を適用してから中心へ足す。

```cpp
FVec3 Offset{};
if ( Context.Random == nullptr
	|| !Context.Random->TryPointInBox3D(
		FVec3{ 5.0f, 2.0f, 8.0f }, Offset ) ) return;
const FVec3 SpawnPosition = AreaCenter + Offset;
```

3軸を独立した一様範囲として選ぶため、箱内部の体積に対して均等になる。全半寸法0は原点を
乱数なしで返す。それ以外は、1軸以上が0で面や線へ退化していても32bit乱数を常に3個進めるため、
同じ種、同じ開始位置、同じ呼出回数なら同じ乱数位置を保てる。負または非有限の成分は出力と
乱数位置を変えずfalseにする。
...
射撃のばらつき、AIの探索方向、指向性のあるエフェクトなど、中心方向から一定角度以内へ散らす場合は
`TryDirectionInCone3D()`を使う。中心軸は任意の長さで渡せ、結果は単位方向として返る。

```cpp
FVec3 ShotDirection{};
if ( Context.Random == nullptr
	|| !Context.Random->TryDirectionInCone3D(
		AimDirection, 3.0f, ShotDirection ) ) return;
```

円錐半角はゲーム側で読みやすい度数法の0〜180度。中心軸との内積、つまり角度のcosを均等に選ぶため、
中心付近へ過度に集中せず立体角へ比例して散らばる。軸の長さは結果へ影響せず、180度は球面全体になる。
0度は正規化した中心軸を乱数なしで返し、0度より大きい成功では32bit乱数を2個進める。
0方向、非有限の軸、範囲外または非有限の角度は、出力と乱数位置を変えずfalseにする。
