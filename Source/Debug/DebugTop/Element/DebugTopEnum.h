...
// -----------------------------------------------------------------------------
// 中身は acs 本体へ入れた foundation/EnumTraits.h と同じ方式で、コンパイラの関数
// シグネチャ文字列から列挙子名を constexpr で切り出す。列挙子を二度書く必要が無く、
// 型を指定するだけで名前が引ける。
//
// ※ これは繋ぎ。配布物 (C:\acs\acs.h) が foundation/EnumTraits.h を含むようになったら
//    本ファイルを消し、acs::TEnumTraits / acs::ToString へ差し替えること。API の形は
//    合わせてあるので、呼び出し側は名前の置換だけで済む。
//
// 使い方:
//   ACS_ENUM()
